...
const BRAND_CATALOG=[
  ...
  {brand:'霸王茶姬',type:'茶',drinks:[['伯牙绝弦',500,60],['桂馥兰香',500,60],['万里木兰',500,55],['青青糯山',500,55],['花田乌龙',500,55],['去云南·玫瑰普洱',500,55],['春日桃桃',500,50],['千山雪金丝小种',500,60]]},
...
let calendarMonth=new Date(),selectedDate=null,selectedSticker=0,photoData='';
const STICKERS=['冰美式','拿铁','手冲','绿茶','珍珠奶茶','葡萄果茶','气泡水','橙汁'];
...
function init(){
  ...
  $('#brand-select').innerHTML='<option value="">选择品牌</option>'+BRAND_CATALOG.map((b,i)=>`<option value="${i}">${b.brand}</option>`).join('');
  ...
  $('#sticker-row').innerHTML=STICKERS.map((name,i)=>`<button type="button" class="sticker-option${i===0?' selected':''}" data-sticker="${i}" aria-label="${name}" style="background-position:${(i%4)*33.333}% ${Math.floor(i/4)*100}%"></button>`).join('');
  ...
  $$('.sticker-option').forEach(b=>b.onclick=()=>selectSticker(+b.dataset.sticker));
...
function quickAdd(p){entries.unshift({id:crypto.randomUUID(),...p,date:new Date().toISOString(),note:'',sticker:stickerForCategory(p.category)});persist();toast(`已记录 ${p.name}`)}
...
function openForm(p=PRESETS[0]){fillPreset(p);selectSticker(stickerForCategory(p.category));photoData='';$('#photo-preview').hidden=true;$('#drink-photo').value='';$('#brand-select').value='';$('#brand-drink-select').innerHTML='<option value="">选择饮品</option>';$('#brand-drink-select').disabled=true;$('#drink-time').value=inputDate();$('#drink-note').value='';$('#drink-dialog').showModal()}
function updateBrandDrinks(){const index=$('#brand-select').value,select=$('#brand-drink-select');if(index===''){select.disabled=true;select.innerHTML='<option value="">选择饮品</option>';return}const brand=BRAND_CATALOG[+index];select.disabled=false;select.innerHTML='<option value="">选择饮品</option>'+brand.drinks.map((d,i)=>`<option value="${i}">${d[0]}</option>`).join('')}
function applyBrandDrink(){const brandIndex=$('#brand-select').value,drinkIndex=$('#brand-drink-select').value;if(brandIndex===''||drinkIndex==='')return;const brand=BRAND_CATALOG[+brandIndex],drink=brand.drinks[+drinkIndex],category=drink[3]||brand.type;$('#drink-name').value=`${brand.brand} · ${drink[0]}`;$('#drink-category').value=category;$('#drink-volume').value=drink[1];$('#drink-caffeine').value=drink[2]}
function stickerForCategory(category){return({咖啡:0,茶:3,水:6,奶类:4,果汁:7,汽水:6,其他:7})[category]??0}
function selectSticker(index){selectedSticker=index;photoData='';$$('.sticker-option').forEach(b=>b.classList.toggle('selected',+b.dataset.sticker===index));if($('#photo-preview'))$('#photo-preview').hidden=true}
function handlePhoto(event){const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const canvas=document.createElement('canvas'),size=320;canvas.width=size;canvas.height=size;const ctx=canvas.getContext('2d'),scale=Math.max(size/img.width,size/img.height),w=img.width*scale,h=img.height*scale;ctx.drawImage(img,(size-w)/2,(size-h)/2,w,h);photoData=canvas.toDataURL('image/jpeg',.72);const preview=$('#photo-preview');preview.src=photoData;preview.hidden=false;$$('.sticker-option').forEach(b=>b.classList.remove('selected'))};img.src=reader.result};reader.readAsDataURL(file)}
...
function saveForm(e){e.preventDefault();entries.unshift({id:crypto.randomUUID(),name:$('#drink-name').value.trim(),category:$('#drink-category').value,volume:+$('#drink-volume').value||0,caffeine:+$('#drink-caffeine').value||0,date:new Date($('#drink-time').value).toISOString(),note:$('#drink-note').value.trim(),emoji:CATEGORIES[$('#drink-category').value],sticker:selectedSticker,photo:photoData});persist();$('#drink-dialog').close();toast('记录已保存')}
function visualHTML(e){if(e.photo)return `<img class="entry-photo" src="${e.photo}" alt="">`;if(Number.isInteger(e.sticker))return `<div class="entry-sticker" style="background-position:${(e.sticker%4)*33.333}% ${Math.floor(e.sticker/4)*100}%"></div>`;return `<div class="entry-icon">${CATEGORIES[e.category]||e.emoji||'🥂'}</div>`}
function entryHTML(e){return `<article class="entry">${visualHTML(e)}<div class="entry-copy"><strong>${esc(e.name)}</strong><small>${formatTime(e.date)}${e.note?` · ${esc(e.note)}`:''}</small></div><div class="entry-values"><strong>${e.volume} ml</strong>${e.caffeine?`<small>${e.caffeine} mg 咖啡因</small>`:''}</div><button class="delete-entry" data-delete="${e.id}" aria-label="删除 ${esc(e.name)}">×</button></article>`}
