...
const BRAND_LOGOS={'瑞幸咖啡':'luckin','库迪咖啡':'cotti','霸王茶姬':'chagee','喜茶':'heytea','农夫山泉':'nongfu'};
const BUILTIN_PRODUCTS=[{id:'americano',name:'标准美式'},{id:'butter',name:'小黄油美式'},{id:'boya',name:'伯牙绝弦'},{id:'orange',name:'橙C美式'},{id:'guifu',name:'桂馥兰香'},{id:'coconut',name:'生椰拿铁'},{id:'grape',name:'多肉葡萄'},{id:'lemon',name:'冰鲜柠檬水'},{id:'mango',name:'杨枝甘露'},{id:'genki',name:'白桃气泡水'},{id:'oriental',name:'东方树叶茉莉花茶'}];
const BRAND_CATALOG=[
  ...
  {brand:'库迪咖啡',type:'咖啡',drinks:[['金奖美式',480,150],['铂金美式',480,150],['潘帕斯蓝生酪拿铁',480,75],['生椰拿铁',480,80,'咖啡','coconut'],['米乳拿铁',480,75],['厚乳拿铁',480,80],['经典美式',480,150],['橙C美式',480,120]]},
  ...
  {brand:'农夫山泉',type:'水',drinks:[['饮用天然水',550,0],['东方树叶茉莉花茶',500,0,'茶','oriental'],['东方树叶青柑普洱',500,0,'茶'],['茶π柚子绿茶',500,25,'茶'],['尖叫运动饮料',550,0,'其他'],['NFC橙汁',300,0,'果汁']]},
  {brand:'元气森林',type:'汽水',drinks:[['白桃味气泡水',480,0,'汽水','genki'],['青柠味气泡水',480,0],['可乐味气泡水',480,0],['外星人电解质水',500,0,'水'],['乳茶',450,25,'奶类']]},
...
let calendarMonth=new Date(),selectedDate=null,photoData='',productVisual='',editingId=null,editingLibraryId=null;
...
function persist(){try{localStorage.setItem('siplog.entries',JSON.stringify(entries))}catch{console.warn('localStorage full; using IndexedDB mirror')}mirrorDurableData();renderAll()}
...
function init(){
  ...
  $('#manage-library').onclick=()=>{renderLibraryManager();$('#library-dialog').showModal()};
...
function catalog(){const groups={};customProducts.forEach(p=>(groups[p.brand]??=[]).push([p.name,p.volume,p.caffeine,p.category,p.photo?`custom:${p.id}`:(p.productVisual||'')]));return [...BRAND_CATALOG,...Object.entries(groups).map(([brand,drinks])=>({brand:`我的 · ${brand}`,rawBrand:brand,type:drinks[0]?.[3]||'其他',drinks}))]}
...
function updateBrandDrinks(){const index=$('#brand-select').value,select=$('#brand-drink-select');if(index===''){select.disabled=true;select.innerHTML='<option value="">选择饮品</option>';showBrandLogo('');return}const brand=catalog()[+index];select.disabled=false;select.innerHTML='<option value="">选择饮品</option>'+brand.drinks.map((d,i)=>`<option value="${i}">${d[0]}</option>`).join('');showBrandLogo(brand.rawBrand||brand.brand)}
function showBrandLogo(brand){const box=$('#brand-logo-preview'),id=BRAND_LOGOS[brand];if(!id){box.hidden=true;return}box.hidden=false;box.querySelector('span').className=`logo-${id}`;box.querySelector('small').textContent=`${brand} · 品牌标识`}
...
function saveToLibrary(){const brand=$('#drink-brand').value.trim(),name=$('#drink-name').value.trim(),button=$('#save-to-library');if(!brand||!name){button.textContent='请先填写品牌和饮品名称';setTimeout(()=>button.textContent='＋ 保存到“我的饮品库”',1800);return}const previous=[...customProducts],existing=editingLibraryId?customProducts.find(p=>p.id===editingLibraryId):customProducts.find(p=>p.brand===brand&&p.name===name),item={id:existing?.id||crypto.randomUUID(),brand,name,category:$('#drink-category').value,volume:+$('#drink-volume').value||0,caffeine:+$('#drink-caffeine').value||0,photo:photoData,productVisual};if(existing)customProducts[customProducts.indexOf(existing)]=item;else customProducts.push(item);try{localStorage.setItem('siplog.customProducts',JSON.stringify(customProducts));editingLibraryId=item.id;refreshBrandSelect();mirrorDurableData();button.textContent='✓ 已保存到“我的饮品库”';button.classList.add('saved');setTimeout(()=>{button.textContent='＋ 保存到“我的饮品库”';button.classList.remove('saved')},1800)}catch{mirrorDurableData();button.textContent='✓ 已保存到设备数据库';button.classList.add('saved');setTimeout(()=>{button.textContent='＋ 保存到“我的饮品库”';button.classList.remove('saved')},1800)}}
function libraryVisualHTML(item){if(item.photo)return `<img src="${item.photo}" alt="">`;if(BUILTIN_PRODUCTS.some(p=>p.id===item.productVisual))return `<div class="product-sticker product-${item.productVisual}"></div>`;return CATEGORIES[item.category]||'🥂'}
function renderLibraryManager(){const root=$('#library-manage-list');root.innerHTML=customProducts.length?customProducts.map(item=>`<article class="library-item"><div class="library-item-visual">${libraryVisualHTML(item)}</div><div class="library-item-copy"><strong>${esc(item.brand)} · ${esc(item.name)}</strong><small>${item.volume||0} ml · ${item.category}</small></div><button type="button" class="library-edit" data-library-edit="${item.id}">编辑</button><button type="button" class="library-delete" data-library-delete="${item.id}">删除</button></article>`).join(''):'<div class="empty"><b>还没有自定义饮品</b><span>记录饮品时可保存到这里</span></div>';root.querySelectorAll('[data-library-edit]').forEach(b=>b.onclick=()=>editLibraryProduct(b.dataset.libraryEdit));root.querySelectorAll('[data-library-delete]').forEach(b=>b.onclick=()=>deleteLibraryProduct(b.dataset.libraryDelete))}
function editLibraryProduct(id){const item=customProducts.find(p=>p.id===id);if(!item)return;editingId=null;editingLibraryId=id;$('#library-dialog').close();$('#drink-dialog header h2').textContent='编辑我的饮品';$('.dialog-save').textContent='完成';$('#drink-brand').value=item.brand;$('#drink-name').value=item.name;$('#drink-category').value=item.category;$('#drink-volume').value=item.volume||0;$('#drink-caffeine').value=item.caffeine||0;$('#drink-time').value=inputDate();$('#drink-note').value='';photoData=item.photo||'';productVisual=item.productVisual||'';const preview=$('#photo-preview');if(photoData){preview.src=photoData;preview.hidden=false;showVisualSource('upload')}else{preview.hidden=true;showVisualSource('builtin');renderProductGallery()}$('#drink-dialog').showModal()}
function deleteLibraryProduct(id){const item=customProducts.find(p=>p.id===id);if(!item||!confirm(`从我的饮品库删除“${item.name}”？`))return;customProducts=customProducts.filter(p=>p.id!==id);localStorage.setItem('siplog.customProducts',JSON.stringify(customProducts));mirrorDurableData();refreshBrandSelect();renderLibraryManager()}
...
function openDurableDb(){return new Promise((resolve,reject)=>{if(!('indexedDB'in window)){reject(Error('IndexedDB unavailable'));return}const request=indexedDB.open('siplog.durable',1);request.onupgradeneeded=()=>request.result.createObjectStore('state');request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)})}
async function mirrorDurableData(){try{const db=await openDurableDb(),tx=db.transaction('state','readwrite');tx.objectStore('state').put({entries,customProducts,updatedAt:new Date().toISOString()},'current');await new Promise((resolve,reject)=>{tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close()}catch(error){console.warn('durable mirror unavailable',error)}}
async function restoreDurableData(){try{const db=await openDurableDb(),tx=db.transaction('state','readonly'),request=tx.objectStore('state').get('current'),state=await new Promise((resolve,reject)=>{request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)});db.close();let restored=false;if(!entries.length&&state?.entries?.length){entries=state.entries;localStorage.setItem('siplog.entries',JSON.stringify(entries));restored=true}if(!customProducts.length&&state?.customProducts?.length){customProducts=state.customProducts;localStorage.setItem('siplog.customProducts',JSON.stringify(customProducts));restored=true}if(restored){refreshBrandSelect();renderAll();toast('已从设备安全存储恢复数据')}else mirrorDurableData()}catch(error){console.warn('durable restore unavailable',error)}}
init();
