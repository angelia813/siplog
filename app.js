...
function saveToLibrary(){const brand=$('#drink-brand').value.trim(),name=$('#drink-name').value.trim(),button=$('#save-to-library');if(!brand||!name){button.textContent='请先填写品牌和饮品名称';setTimeout(()=>button.textContent='＋ 保存到“我的饮品库”',1800);return}const previous=[...customProducts],existing=customProducts.find(p=>p.brand===brand&&p.name===name),item={id:existing?.id||crypto.randomUUID(),brand,name,category:$('#drink-category').value,volume:+$('#drink-volume').value||0,caffeine:+$('#drink-caffeine').value||0,photo:photoData,productVisual};if(existing)customProducts[customProducts.indexOf(existing)]=item;else customProducts.push(item);try{localStorage.setItem('siplog.customProducts',JSON.stringify(customProducts));refreshBrandSelect();button.textContent='✓ 已保存到“我的饮品库”';button.classList.add('saved');setTimeout(()=>{button.textContent='＋ 保存到“我的饮品库”';button.classList.remove('saved')},1800)}catch{customProducts=previous;button.textContent='设备存储空间不足';setTimeout(()=>button.textContent='＋ 保存到“我的饮品库”',1800)}}
