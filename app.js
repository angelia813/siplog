...
function init(){
  ...
  $('#sticker-row').innerHTML=STICKERS.map((name,i)=>`<button type="button" class="sticker-option${i===0?' selected':''}" data-sticker="${i}" aria-label="${name}" style="background-position:${-(i%4)*100}% ${-Math.floor(i/4)*100}%"></button>`).join('');
...
function visualHTML(e){if(e.photo)return `<img class="entry-photo" src="${e.photo}" alt="">`;if(Number.isInteger(e.sticker))return `<div class="entry-sticker" style="background-position:${-(e.sticker%4)*100}% ${-Math.floor(e.sticker/4)*100}%"></div>`;return `<div class="entry-icon">${CATEGORIES[e.category]||e.emoji||'🥂'}</div>`}
